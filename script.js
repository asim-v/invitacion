...
class WTCGL {
  ...
  * @param  {Image} image The image object to add to the texture
  ...
  run(delta) {
    ...
    const runFunction = () => {
      this.time = this.startTime + delta * .00004;
      this.onRun(delta);
      this.render();
    };
...
window.addEventListener('pointermove', e => {
  ...
  if (window.innerHeight > window.innerWidth) {
    mousepos[0] = ((e.pageX - window.innerWidth / 2) / window.innerWidth)* sensitivity;
    mousepos[1] = ((e.pageY - window.innerHeight / 2) / window.innerHeight * -1 * ratio)* sensitivity;
  } else {
    mousepos[0] = ((e.pageX - window.innerWidth / 2) / window.innerWidth / ratio)* sensitivity;
    mousepos[1] = ((e.pageY - window.innerHeight / 2) / window.innerHeight * -1)* sensitivity;
  }
